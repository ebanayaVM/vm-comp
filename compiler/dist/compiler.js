...
class Compiler {
    constructor(src) {
        ...
        this.dependencies = ['console', 'Array', 'navigator', 'eval', 'fetch', 'XMLHttpRequest']; // Add 'XMLHttpRequest'
        this.contexts = [{ variables: new Map(), counter: 0 }];
        this.contexts[0].variables.set('node.name', '');
        var block = { instructions: [], inheritsContext: true };
        this.blocks = [block];
        this.ir = { 'main': block };
    }
    ...
    createXMLHttpRequest() {
        return { type: 'XMLHTTPREQUEST', value: null };
    }
    translateFunctionDeclaration(node) {
        var dst = this.contexts[0].counter++;
        this.appendHeaderInstruction('function'); // Добавляем заголовок функции
        this.appendStoreInstruction([this.createNumberArgument(dst), this.translateFunctionExpression(node)]);
        this.initalizeVariable(node.id.name, dst);
    }

    translateFunctionExpression(node) {
        var block = { instructions: [], inheritsContext: true };
        this.blocks.unshift(block);
        this.constructIR(node.body.body);
        this.blocks.shift();
        return { type: 'function', value: block };
    }
    translateXMLHttpRequest(node) {
        const dst = this.contexts[0].counter++;
        this.appendStoreInstruction([this.createNumberArgument(dst), this.createXMLHttpRequest()]);
        ...
        switch (node.type) {
            ...
            case "CallExpression":
                if (node.callee.type === "Identifier" && node.callee.name === "fetch") {
                    this.pushFetchCallOntoStack(node);
                } else if (node.callee.type === "Identifier" && node.callee.name === "XMLHttpRequest") {
                    this.pushXMLHttpRequestCallOntoStack(node);
                } else {
            ...
            case "BooleanLiteral": // Добавьте эту часть для обработки BooleanLiteral
            ...
            case "ArrowFunctionExpression":
                var dst = this.contexts[0].counter++;
                this.appendStoreInstruction([this.createNumberArgument(dst), this.translateExpression(node.body)]);
                return this.createVariableArgument(dst);
            case "BlockStatement":
                this.constructIR(node.body);
                break;
            case "NewExpression":
                if (node.callee.name === "XMLHttpRequest") {
                    this.appendNewXMLHttpRequestInstruction();
                    const variableIndex = this.contexts[0].counter++;
                    this.appendPopInstruction(this.createNumberArgument(variableIndex));
                    return this.createVariableArgument(variableIndex);
                }
    ...
    appendHeaderInstruction(header) {
        switch (header) {
            case 'function':
                this.pushInstruction({ opcode: 'FUNCTION_HEADER', args: [] });
                break;
    ...
    appendOpenInstruction() {
        const instruction = { opcode: 'OPEN', args: [] };
        this.pushInstruction(instruction);
    }
    appendStrictEqualInstruction() {
        const instruction = { opcode: 'STRICT_EQUAL', args: [] };
        this.pushInstruction(instruction);
    }
    ...
    appendNewXMLHttpRequestInstruction() {
        this.pushInstruction({ opcode: "NEW_XMLHTTPREQUEST", args: [] });
    }
    ...
    translateBinaryExpression(node) {
        ...
        switch (node.operator) {
            case "==":
                ...
                break;
            case "===": // Добавьте эту часть для обработки оператора ===
                this.appendStrictEqualInstruction();
                break;
            case "+":
    ...
    pushXMLHttpRequestCallOntoStack(node) {
    ...
    pushMemberExpressionOntoStack(node) {
        switch (node.object.type) {
            case "Identifier":
                ...
                } else if (node.object.name === "xhr") { // Добавьте эту часть для обработки xhr
                    var reg = this.contexts[0].variables.get(node.object.name);
                    if (reg == undefined) {
                        throw "UNKNOWN_SOURCE_VARIABLE";
                    }
    ...
    translateVariableDeclarator(declaration) {
        // Реализация функции
        // Пример:
        const dst = this.contexts[0].counter++;
        this.initalizeVariable(declaration.id.name, dst);
        this.appendStoreInstruction([this.createNumberArgument(dst), this.translateExpression(declaration.init)]);
    }
        ...
        for (var i = 0; i < statements.length; i++) {
            ...
            switch (node.type) {
                ...
                case "ExpressionStatement":
                    this.translateExpression(node.expression);
                    break;
                case "FunctionDeclaration": // Добавляем обработку FunctionDeclaration
                    this.translateFunctionDeclaration(node);
                    break;
                default:
                    console.log(node.type);
